...
export const Sidebar = () => {
  ...
  // Desktop: click outside to auto-hide (collapse) sidebar
  useEffect(() => {
    const handleOutsideClick = (event: MouseEvent) => {
      ...
      if (desktopSidebarRef.current && !desktopSidebarRef.current.contains(target)) {
      ...
      <div className="p-6 border-b border-sidebar-border">
        <div className="flex items-center justify-between gap-2">
          ...
          {!mobile && (
            ...
            >
              {compactNav ? <PanelLeft size={18} /> : <PanelLeftClose size={18} />}
            </Button>
          )}
        </div>
      </div>
